...
md"""
# OLP - reward generated using two-regime mixed distributions
### June, 2023
"""
...
# ╔═╡ 935876d9-bfea-49ab-b39f-d0909241b734
begin
	Random.seed!(1234) 
	...
	# Parameters for the time-varying trend
	trend_slope = 0.001
	trend_intercept = 0.02
	
	# Parameters for the time-varying variance
	variance_slope = 0.001
	variance_intercept = 0.01
	
	# Create normal distributions for each regime
	regime1_dist = Normal(regime1_mean, regime1_std)
	regime2_dist = Normal(regime2_mean, regime2_std)
	
	# Generate regime labels based on the probability
	regime_labels = rand(Bernoulli(regime1_prob), n)
	
	# Generate financial data based on the regime labels, trend, and variance
	r = zeros(n)
	for i in 1:n
	    trend = trend_slope * i + trend_intercept
	    variance = variance_slope * i + variance_intercept
	    
	    if regime_labels[i] == 1
	        r[i] = trend + rand(Normal(0, sqrt(variance))) + rand(regime1_dist)
	    else
	        r[i] = trend + rand(Normal(0, sqrt(variance))) + rand(regime2_dist)
	    end
	end
	
	# Plot the generated data
	plot(r, xlabel="Time/Bidder", ylabel="Reward", legend=false)
	
end

# ╔═╡ 733e2bef-e469-46e6-b072-ff0214787506
begin
...
begin
	...
	x = oneTimeOlp(lpmodel_25, 25, A, b, r, n)
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a1d1b2752913a3073094238ab11a261114c17057"
